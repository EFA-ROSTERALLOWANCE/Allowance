...
const base = { date:'2026-06-01', signOn:'21:50', origin:'WSI', crew:2, basis:'live',
               extraHour:false, leadIn:60, prevOdpH:null, sectors:secs };
...
eq(M.fmtDur(M.computeDuty({...base, extraHour:true}).fdpLimMin), '13:00', 'the extra hour adds 1:00');
...
for(const variant of [{}, {basis:'planning'}, {extraHour:true}, {crew:3}]){
...
const dh = M.computeDuty({ date:'2026-06-01', signOn:'06:00', origin:'SYD', crew:2, basis:'live',
  extraHour:false, leadIn:60, prevOdpH:null, sectors:[
...
for(const pat of M.PATTERNS){
  ...
  const r = M.computeDuty({ date:'2026-06-01', signOn:pat.rpt || '08:00', origin:pat.origin,
    crew:pat.crew || 2, basis:'live', extraHour:false, leadIn, prevOdpH:null, sectors:ss });
